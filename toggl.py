...
class DateAndTime(object):
    ...
    def start_of_7days_ago(self):
        """
        Returns 00:00:00 7 days ago as a localized datetime object.
        """
        return self.tz.localize(
            datetime.datetime.combine( datetime.date.today(), datetime.time.min) - 
            datetime.timedelta(days=7) # subtract 7 days from today at midnight
        )
...
class TimeEntryList(object):
    ...
    def reload(self):
        ...
        url = "%s/time_entries?start_date=%s&end_date=%s" % \
            (TOGGL_URL, urllib.parse.quote(DateAndTime().start_of_7days_ago().isoformat('T')), \
